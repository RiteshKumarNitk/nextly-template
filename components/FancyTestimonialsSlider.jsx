...
export default function FancyTestimonialsSlider({ testimonials }) {
  ...
  useEffect(() => {
    if (!autorotate || !testimonials?.length) return; // Add check for testimonials length
    const interval = setInterval(() => {
      setActive(prevActive => (prevActive + 1) % testimonials.length);
    }, autorotateTiming);
    return () => clearInterval(interval);
  }, [autorotate, testimonials?.length]); // Add testimonials.length to dependency array
    ...
    <div className="w-full max-w-3xl mx-auto text-center">
      ...
      <div className="relative h-32">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[480px] h-[480px] pointer-events-none before:absolute before:inset-0 before:bg-gradient-to-b before:from-indigo-500/25 before:via-indigo-500/5 before:via-25% before:to-indigo-500/0 before:to-75% before:rounded-full before:-z-10">
          <div className="h-32 mask-image-linear-gradient(0deg,transparent,theme(colors.white)_20%,theme(colors.white))">

            {testimonials?.map((testimonial, index) => ( // Add optional chaining here
              <Transition
                key={index}
                show={active === index}
